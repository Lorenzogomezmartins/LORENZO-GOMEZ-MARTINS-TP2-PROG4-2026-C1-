...
let UsuariosService = class UsuariosService {
    ...
    async crearUsuarioDesdeAdmin(dto) {
        const correoExistente = await this.usuarioModel.findOne({
            correo: dto.correo,
        });
        if (correoExistente) {
            throw new common_2.BadRequestException('El correo ya está registrado');
        }
        const nombreUsuarioExistente = await this.usuarioModel.findOne({
            nombreUsuario: dto.nombreUsuario,
        });
        if (nombreUsuarioExistente) {
            throw new common_2.BadRequestException('El nombre de usuario ya está registrado');
        }
